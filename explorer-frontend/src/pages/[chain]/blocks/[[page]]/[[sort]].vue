  ...
  <div>
    <div class="flex pb-4 pt-6 justify-between">
      ...
      <div class="text-right">
        <a
          :href="buildRouteSort('desc')" class="
              uppercase
              text-sky-700
              dark:text-sky-400
              hover:underline
    ...
    <div class="rounded p-4 bg-gray-50 dark:bg-gray-800">
      <SharedBlocksTable :data="blocks" :reactivity-fix="reactivityFix" />
    </div>
    <SharedPagination
      :overall-entries="backgroundInfoDataState != null
        && backgroundInfoDataState.currentSyncedBlock != null
        ? backgroundInfoDataState.currentSyncedBlock
        : 0
      " :entries-per-page="config.public.blocksPerPage" :current-page="currentPage"
...
import { useBackgroundInfo, useLatestBlockInfo } from "@/composables/States";
...
watch(latestBlock, (nval) => {
  if (nval === null)
    return;
  if (currentPage.value > 1 || targetSort.value === 0)
    return;
  blocks.value.pop();
