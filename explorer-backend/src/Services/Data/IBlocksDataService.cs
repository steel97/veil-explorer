...
public interface IBlocksDataService
{
    ...
    public Task<Block?> GetBlockAsync(string hash, int simplifiedTxInfo = 1, CancellationToken cancellationToken = default);
    public Task<Block?> GetBlockAsync(int height, int simplifiedTxInfo = 1, CancellationToken cancellationToken = default);
